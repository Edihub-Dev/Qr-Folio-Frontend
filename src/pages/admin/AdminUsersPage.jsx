...
const AdminUsersPage = () => {
  ...
  const openModal = async (type, userId) => {
    try {
      ...
      if (type === "renew") {
        setRenewForm((prev) => ({
          planName: response?.data?.planName || response?.data?.subscriptionPlan || "",
  ...
  const closeModal = () => {
    setModalState({ view: false, edit: false, confirm: false, remind: false, renew: false, logs: false, expiry: false });
    setSelectedUser(null);
    setPendingAction(null);
    setReminderMessage("");
    setRenewForm({ planName: "", paymentMethod: "phonepe", durationInMonths: 12 });
    setReminderLogs([]);
  ...
  const handleRefreshSubscription = async (user) => {
    try {
      ...
      setError(err?.response?.data?.message || err.message || "Failed to refresh subscription");
    } finally {
  ...
  const handleSendReminder = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || err.message || "Failed to send reminder");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleRenewSubscription = async () => {
    ...
    try {
      setActionLoading(true);
      await renewAdminUserSubscription(selectedUser.id || selectedUser._id, renewForm);
      closeModal();
      refresh();
    } catch (err) {
  ...
  const handleUpdateExpiry = async () => {
    ...
    try {
      setActionLoading(true);
      await updateAdminUserSubscriptionExpiry(selectedUser.id || selectedUser._id, {
        planExpireDate: expiryForm.planExpireDate ? new Date(expiryForm.planExpireDate).toISOString() : null,
        notes: expiryForm.notes,
      });
      closeModal();
      refresh();
    } catch (err) {
      setError(err?.response?.data?.message || err.message || "Failed to update expiry");
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const loadReminderLogs = useCallback(async (userId) => {
    try {
      ...
      setError(err?.response?.data?.message || err.message || "Failed to load reminder logs");
    } finally {
    ...
    <div className="space-y-6">
      ...
      >
        {selectedUser ? (
          <div className="space-y-4 text-sm">
            <p>
              The reminder email will be sent to <strong>{selectedUser.email}</strong>.
              Provide an optional note to include in the log.
            </p>
            <textarea
          ...
          <div className="grid gap-4 text-sm">
            <label className="flex flex-col gap-1 text-xs font-medium text-gray-500">
              ...
              <input
                ...
                onChange={(event) =>
                  setRenewForm((prev) => ({ ...prev, planName: event.target.value }))
                }
            ...
            <label className="flex flex-col gap-1 text-xs font-medium text-gray-500">
              ...
              <select
                ...
                onChange={(event) =>
                  setRenewForm((prev) => ({ ...prev, paymentMethod: event.target.value }))
                }
                className="rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-100"
              >
      ...
      >
        {logsLoading ? (
          ...
          <div className="text-sm text-gray-500">No reminders logged for this user yet.</div>
        ) : (
          <ul className="space-y-3 text-sm">
            {reminderLogs.map((log) => (
              <li key={log._id || log.id} className="rounded-xl border border-gray-200 bg-slate-50 p-3">
                <div className="flex items-center justify-between">
                ...
                <div className="mt-1 text-xs text-gray-500">
                  Plan: {log.planName || "—"} (expires {log.planExpireDate ? new Date(log.planExpireDate).toLocaleDateString() : "—"})
                </div>
                {log.notes && (
        ...
        {selectedUser ? (
          <div className="grid gap-4 text-sm">
            ...
            <p className="rounded-lg bg-amber-50 p-3 text-xs text-amber-700">
              Setting a past date will immediately mark the plan as expired. Upcoming dates refresh reminders and set the plan back to active.
            </p>
          </div>
        ) : (
          <div className="text-sm text-gray-500">Loading user...</div>
